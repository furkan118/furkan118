...
tok = input(' ☆ {}TOKEN {}GİR {}OC: {}'.format(B,C,V,K))
print("\r")
iD = input(' ☆ {}ID{}  {}GİR {}AMCIK : {}'.format(B,C,V,K))
os.system('clear')
def cookie(email):
...
def date_sc(Id):
 try:
  if int(Id) >1 and int(Id)<1279000:
   ...
   return "2020-2023"
 except BaseException as WOTJEX :
  return WOTJEX
...
def check_email(email):
	...
	bo = f'\x1b[38;5;{b}m'
	check+=1
	sys.stdout.write(f"\r   {bo}[ {C}WOTJEX {bo}] {C}Good Hit : {F}{good_hot}  {C}Bad IG : {R}{bad_ig}  {C}Good IG : {X}{good_ig}  {C}{bo}Check•{check}\r")
	sys.stdout.flush()
