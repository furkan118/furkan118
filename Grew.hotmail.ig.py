...
def info(username,jj):
  ...
  try:
    ...
    try:
      ...
      date='None'
    tlg = f'''
⌯ OLUM HADİ YİNE İYİSİN HE AL HİTİNİ
ᯓᯓᯓᯓᯓᯓᯓᯓ
TAKIPCI : {user_followers}
Takip ettigi : {user_following}
posty : {total_posts}
kul. adi : {username}
email : {username}@{jj}
tarih  : {date}
...
def qqq():
  while True:
    try:
      ...
      username = requests.post('https://www.instagram.com/api/graphql', headers=headers, data=data).json()['data']['user']['username']#['@BenSwozy']
